...
function viewer(pdfUrl){
	// 判断是否是ppt格式的，顺便取出转成pdf后的文件
	var isPPT = false;
	var arrPdf = pdfUrl.split('.');
	if(arrPdf.length > 1) {
		sourceFormat = arrPdf[arrPdf.length -1];
		if(sourceFormat.toLowerCase() == 'ppt' || sourceFormat.toLowerCase() == 'pptx'){
			document.getElementById('viewerContainer').style.overflowY = 'hidden';
			document.getElementById('previousPPT').style.display = 'table';
			document.getElementById('nextPPT').style.display = 'table';
			isPPT = true;
		}
	...
	function setPPT(){
		if(isPPT){
    		...
    		if(parseInt(allPageCount,10) == 1){
    			document.getElementById('previousPPT').style.display = 'none';
    			document.getElementById('nextPPT').style.display = 'none';
    		} else {
    			if(parseInt(pageNumber,10) == 1){
        			document.getElementById('previousPPT').style.display = 'none';
        			document.getElementById('nextPPT').style.display = 'table';
        		} else if(parseInt(pageNumber,10) == parseInt(allPageCount,10)){
        			document.getElementById('nextPPT').style.display = 'none';
        			document.getElementById('previousPPT').style.display = 'table';
        		} else {
        			document.getElementById('previousPPT').style.display = 'table';
        	...
        	var curPage = document.getElementById('pageContainer' + pageNumber);
        	var pageHeight = parseInt(curPage.style.height,10);
        	document.getElementById('viewerContainer').style.height = (pageHeight-7) + 'px';
	    	document.getElementById('outerContainer').style.height = (pageHeight+25) + 'px'; 
    	} 
	}
	
	function configure(PDFJS) {
	  PDFJS.imageResourcesPath = './images/';
	  PDFJS.workerSrc = baseUrl + '/lar-ui/lar-pdf/build/pdf.worker.js';
	...
	var PDFViewer = (function pdfViewer() {
	  ...
	  PDFViewer.prototype = /** @lends PDFViewer.prototype */{
	    ...
	    setDocument: function (pdfDocument) {
	      ...
	      allPageCount = pdfDocument.numPages;
	      var self = this;
	        	...
	        	/*if(isPPT){
	        		...
	        		if(parseInt(pagesCount,10) == 1){
	        			...
	        			if(parseInt(pageNumber,10) == 1){
		        			document.getElementById('previousPPT').style.display = 'none';
		        			document.getElementById('nextPPT').style.display = 'table';
		        		} else if(parseInt(pageNumber,10) == parseInt(pagesCount,10)){
		        			document.getElementById('nextPPT').style.display = 'none';
		        			document.getElementById('previousPPT').style.display = 'table';
		        		} else {
		        			document.getElementById('previousPPT').style.display = 'table';
		        	...
		        	var curPage = document.getElementById('pageContainer' + pageNumber);
		        	var pageHeight = parseInt(curPage.style.height,10);
		        	document.getElementById('viewerContainer').style.height = (pageHeight-10) + 'px';
			    	document.getElementById('outerContainer').style.height = (pageHeight+22) + 'px';
	        	}*/
	...
	var PDFViewerApplication = {
	  ...
	  load: function pdfViewLoad(pdfDocument, scale) {
	    ...
	    pdfDocument.getMetadata().then(function(data) {
	      ...
	      if (pdfTitle) {
	        self.setTitle(pdfTitle + ' - ' + this.title);
	      }
	...
	var HOSTED_VIEWER_ORIGINS = ['null',
	  'http://mozilla.github.io', 'https://mozilla.github.io', 'http://localhost:8080'];
	...
	function webViewerInitialized() {
	  ...
	  var container = 'container' in params ? params.container : '';
	  ...
	  document.getElementById('pdfContainer').appendChild(fileInput);
	  ...
	  document.getElementById('previousPPT').addEventListener('click',
		function() {
		  PDFViewerApplication.page--; 
		  setPPT();
	  });
			
	  document.getElementById('nextPPT').addEventListener('click',
	    function() {
	      PDFViewerApplication.page++;
	      setPPT();
	  }); 
	  ...
	  document.getElementById('skinSelect').addEventListener('change', function() {
		  document.getElementById('toolbarContainer').style.background=this.value;
	      document.getElementById('viewerContainer').style.background=this.value;
	      document.getElementById('toolbarSidebar').style.background=this.value;
	      document.getElementById('sidebarContent').style.backgroundColor=this.value;
	      document.getElementById('findbar').style.background=this.value;
	      document.getElementById('secondaryToolbar').style.background=this.value;
	  });
	...
	(function animationStartedClosure() {
	  // The offsetParent is not set until the larPdf.js iframe or object is visible.
