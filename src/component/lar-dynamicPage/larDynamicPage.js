...
(function($){
            ...
            $.ajax({
                ...
                success:function(data){
                	...
                	_this.$container.append("<div class='lar-dynamicPage'>" +
                			...
                			"		<ul style='display:" + ((_this.options.pageHide && _this.pageNums<=1) || _this.options.pagingMethod != 'manual'?'none':'inline-block') + "'></ul>" +
        					"	</div>" +
        					"</ul>" +
        ...
        dynamicPage.prototype.css=function(index){
            ...
            if(_this.curIndex % 4 == 0){
            	$(_this.$pagesList).find("li :eq(4)").parent().addClass("active");
            }else{
            	$(_this.$pagesList).find("li :eq("+(_this.curIndex % 4)+")").parent().addClass("active");
            }
          ...
          if(_this.curIndex>=_this.options.maxPage) {	// 如果页数到3位数了，就不显示页数，只显示翻页
        	  ...
        	  objPrev.css({width:'20px'});
        	  objNext.css({width:'20px'});
        	  objPrev.find('span').text('<');
        	  objNext.find('span').text('>'); 
        	  $(_this.$pagesList).find("li.page").css({width:'20px'});
...
//                    scroll(0,400);
                    ...
                    }else if(index=="more" || index=="go" || index=="totalNum" || index=="total"){
        ...
        function buildPageListDom(allDataCount){
            ...
            if(allDataCount && myThis.pageNums>0){
                if(myThis.windowWidth<768){
                	var $pageNum=$("<li class='pageNum' index=prev><span><</span></li><li class='pageNum' index=next><span>></span></li>");
//                	$pageNum.wrap('<p/>').parent().html()
                	$(myThis.$pagesList).append($pageNum);
                	$(myThis.$pagesList).parents('.lar-dynamicPage').find('.nextPageContainer .mobilePageInfo').html('1/' + myThis.pageNums);
                }else{                    
                    var showAll=0;
                    ...
                    for(var k=1;k<(showAll+2);k++){
                        ...
                        if(k==(showAll+1)){                        	
                            ...
                            $(myThis.$pagesList).append("<li class='total' index=total><span>共"+myThis.pageNums+"页</span></li>");
        ...
        function prevPage(){
        	...
        	if( _this.curIndex > 1){
                 ...
                 $.when(updateResultDom.apply(_this,[{curPageNum:_this.curIndex-1,dtd:dtd}])).done(function(){
                         if(_this.windowWidth>768 && _this.curIndex % 4 == 1 && _this.pageTurn>0){
                             //页码区该翻页了
                             _this.pageTurn--;
                             _this.isLastTurn=false;
                             $(_this.$pagesList).find("li.page").remove();
                             for(var i=_this.curIndex-1; i>_this.curIndex-5; i--){
        ...
        function nextPage(){
        	var _this=this;
        	 if(_this.curIndex < _this.pageNums){
                 ...
                 $.when(updateResultDom.apply(_this,[{curPageNum:_this.curIndex+1,dtd:dtd}])).done(function(){
                     //满足一下条件，页码区该翻页了
                     if(_this.windowWidth>768 && _this.curIndex % 4 == 0 && _this.pageTurn < Math.ceil(_this.pageNums/4)){
                         _this.pageTurn++;
                         _this.isLastTurn=false;
                         var lastTurn=_this.pageNums-_this.curIndex;
                         if(lastTurn>=4){
                             for(var i=0;i<4;i++){
                                 $(_this.$pagesList).find("li span:eq("+(i+1)+")").text(i+_this.pageTurn*4+1);
                             }
                         }else{
                             for(var i=0;i<4;i++){
        ...
        function gotoPage(){
        	var _this=this;
             ...
             $.when(updateResultDom.apply(_this,[{curPageNum:fillPageNum,dtd:dtd}])).done(function(){
                 ...
                 //重置页码区
                 var theIndex=fillPageNum%4;
                 _this.pageTurn=Math.ceil(fillPageNum/4)-1;
                 var lastTurn=_this.pageNums-fillPageNum;
                 var turnNums=Math.ceil(_this.pageNums/4)-1;
                 if(lastTurn>=4 || _this.pageTurn < turnNums){
                     //页码区该翻页了
                     updatePageDom(_this);
                 }else if( !_this.isLastTurn){
                     var lastNums=_this.pageNums-_this.pageTurn*4;
                     for(var i=0;i<4;i++){
                         if(i<lastNums){
                             $(_this.$pagesList).find("li span:eq("+(i+1)+")").text(i+_this.pageTurn*4+1);
                         }else{
